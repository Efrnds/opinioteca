...
export default function MenuEsquerdo() {
    ...
    return (
        <section className="flex h-full w-full flex-col gap-11 overflow-hidden">
            <Box className="flex h-full w-full flex-col items-start justify-between gap-8">
                <div className="flex w-full flex-col items-start gap-8">
                    <h1 className="font-gabarito-bold text-4xl text-azul-900">Menu</h1>
                        ...
                        {itensMenu.map(({ href, rotulo, icone: Icone }) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="font-gabarito-bold text-xl">{rotulo}</span>
                                </Link>
                            );
                        })}
                    </nav>
                ...
                >
                    ...
                    <span className="hidden font-gabarito-bold text-xl lg:inline">Nova Resenha</span>
                </button>
            </Box>

            <NovaAvaliacaoModal open={modalAberto} onClose={() => setModalAberto(false)} />

            <Box ref={contaRef} className="relative flex w-full items-center justify-between gap-2 p-4">
                <Link href="/perfil" className="flex min-w-0 items-center gap-2">
                    {session?.user?.image ? (
                        ...
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gray-200 font-gabarito-bold text-2xl text-azul-900">
                            {nick.charAt(0).toUpperCase()}
                        </div>
                    )}
                    <span className="truncate font-gabarito-bold text-xl text-azul-900">{nick}</span>
                </Link>
